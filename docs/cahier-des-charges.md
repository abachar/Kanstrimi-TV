...
> Document d'origine, conservé tel quel. Les fonctionnalités décrites restent la cible ;
> la pile technique a changé (Node + Hono, Postgres, admin Hono JSX + HTMX, API
> Xtream-compatible avant l'API REST). L'état réel du projet est dans le `README.md`
> à la racine et dans `server/BACKLOG.md`.
